...
runtime_seconds = 7200  # total simulation time
sleep_time = 0.2  # seconds between updates (set 60 for real minutes, set smaller for testing)

# --- Simulation Functions ---

def update_price(S_prev, mu, sigma, dt=1/390):
    """Update price using GBM."""
    dW = np.random.normal(0, np.sqrt(dt))
    S_new = S_prev * np.exp((mu - 0.5 * sigma**2) * dt + sigma * dW)
    return S_new

# --- Main Simulation Loop ---

price_history = {ticker: [params["S0"]] for ticker, params in stocks.items()}
current_prices = {ticker: params["S0"] for ticker, params in stocks.items()}

plt.ion()  # Turn on interactive mode
fig, axes = plt.subplots(3, 2, figsize=(15, 8))
axes = axes.flatten()

for minute in range(1, runtime_seconds + 1):
    print(f"\n--- Minute {minute} ---")
